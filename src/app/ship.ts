...
class Ship extends Drawable {
    ...
    accelerate(delta: number) {
        ...
        const kick = 1.5
        const thrustFactor =
            Settings.SPEED_FACTOR * delta * (this.speed.equals(Pair.origin()) ? thrustBase * kick : thrustBase)
    ...
    draw() {
        ...
        const destroyedRotation = this.isDestroyed() ? (this.collisionStartFrame * 0.02) % (2 * Math.PI) : NaN
